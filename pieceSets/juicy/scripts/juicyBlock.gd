...
func runAnimation():
	if piece.state == piece.IDLE_STATE and lastState == piece.LAND_STATE:
		set_current_animation("idleLand")
	
	elif piece.state == piece.IDLE_STATE and lastState == piece.INCREMENT_STATE:
		set_current_animation("idleIncrement")
	
	elif piece.state == piece.IDLE_STATE and lastState == piece.SWITCH_LEFT_STATE:
		set_current_animation("idleLeft")
	
	elif piece.state == piece.IDLE_STATE and lastState == piece.SWITCH_RIGHT_STATE:
		set_current_animation("idleRight")
	
	elif piece.state == piece.IDLE_STATE:
